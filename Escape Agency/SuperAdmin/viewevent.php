...
//The details of the event
    ...
    //get Event details
    $result = mysqli_query($conn,"SELECT * FROM Events WHERE EventID = $id");
    ...
    if (isset($_POST['update'])) {
      ...
      if (isset($_FILES['img']) && $_FILES['img']['error'] === 0) {
        ...
        // SQL update with or without image
          if ($target_file) {
            $stmt = $conn->prepare("UPDATE Events SET Name=?, Description=?, Tagline=?, Activities=?, Location=?,Price=?, Status=?, StartDate=?, EndDate=?,ImageURL=? WHERE EventID=? ");
            $stmt->bind_param("sssssdsisss", $Name, $desc, $tag, $activities, $location,  $price, $status, $Sdate, $Edate, $target_file, $id);
        } else {
            $stmt = $conn->prepare("UPDATE Events SET Name=?, Description=?, Tagline=?, Activities=?, Location=?,Price=?, Status=?, StartDate=?, EndDate=? WHERE EventID=? ");
        ...
        $stmt = $conn->prepare("UPDATE Events SET Status = 'inactive' WHERE EventID = ?  ");
        ...
        $stmt = $conn->prepare("UPDATE Events SET Status = 'active' WHERE EventID = ?  " );
        $stmt->bind_param("i", $id);
        if ($stmt->execute()) {
