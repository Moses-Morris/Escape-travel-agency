...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // --- Update Property ---
        ...
        if ($target_file) {
            $stmt = $conn->prepare("UPDATE agentproperties 
                ...
                WHERE PropertyID=?");
            $stmt->bind_param("ssssssssssi", $name, $agentID, $status, $services, $feature, $desc, $price, $location, $type, $target_file, $propid);
        } else {
            $stmt = $conn->prepare("UPDATE agentproperties 
                SET PropertyName=?, AgentID=?, Status=?, Services=?, Features=?, Description=?, Price=?, Location=?, OptionType=? 
                WHERE PropertyID=? ");
            $stmt->bind_param("ssssssssii", $name, $agentID, $status, $services, $feature, $desc, $price, $location, $type, $propid);
        }
    ...
    elseif (isset($_POST['deactivate'])) {
        $stmt = $conn->prepare("UPDATE agentproperties SET Status = 'inactive' WHERE PropertyID = ? ");
    ...
    elseif (isset($_POST['delete'])) {
        $stmt = $conn->prepare("DELETE FROM agentproperties WHERE PropertyID = ? ");
        $stmt->bind_param("i", $propid);
        if ($stmt->execute()) {
            echo "<div class='alert alert-success'>Property deleted successfully.</div>";
            echo "<script>
                    setTimeout(function() {
                        window.location.href = 'hostings.php';
                    }, 3000);
                  </script>";
            exit;
        } else {
                  ...
                  <div class="card-body">
                    ...
                    <form class="forms-sample" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?propid=' . $id; ?>" method="post" enctype="multipart/form-data">
                      ...
                      <?php
                          ...
                          $result = mysqli_query($conn,"SELECT * FROM agentproperties WHERE PropertyID = $id");
