...
if ($destid > 0) {
    $result = mysqli_query($conn, "SELECT * FROM destinations d 
                                  ...
                                  WHERE a.AgentID = $agentID AND d.destinationID = $destid");
    ...
    $gallery_result = mysqli_query($conn, "SELECT * FROM DestinationGallery WHERE DestinationID = $destid");
    if ($gallery_row = mysqli_fetch_array($gallery_result)) {
        $existing_images = array(
            ...
            'gallery_desc' => $gallery_row['Description']
        );
    }
}

// Handle form submissions
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $destid > 0) {
    ...
    if (isset($_POST['create'])) {
        ...
        if (isset($image_paths['main']) && $image_paths['main'] !== false) {
            ...
            $insert_query = "INSERT INTO DestinationGallery 
                           (DestinationID, Description, Added_By, Image1, Image2, Image3, Image4, Image5) 
                           VALUES 
                           ($destid, '$description', '$added_by', '$img1', '$img2', '$img3', '$img4', '$img5')";
            
            if (mysqli_query($conn, $insert_query)) {
                $msg = "<div class='alert alert-success'>Images uploaded successfully!</div>";
                // Refresh existing images
                $gallery_result = mysqli_query($conn, "SELECT * FROM DestinationGallery  WHERE DestinationID = $destid ORDER BY ID DESC LIMIT 1");
                if ($gallery_row = mysqli_fetch_array($gallery_result)) {
                    $existing_images = array(
                        'main' => $gallery_row['Image1'],
                        'img1' => $gallery_row['Image2'],
    ...
    if (isset($_POST['update']) && isset($existing_images['gallery_id'])) {
        ...
        if (!empty($update_fields)) {
            $update_query = "UPDATE DestinationGallery SET " . implode(', ', $update_fields) . " WHERE ID = $gallery_id";
            
            if (mysqli_query($conn, $update_query)) {
                $msg = "<div class='alert alert-success'>Images updated successfully!</div>";
                // Refresh existing images
                $gallery_result = mysqli_query($conn, "SELECT * FROM DestinationGallery  WHERE ID = $gallery_id");
                if ($gallery_row = mysqli_fetch_array($gallery_result)) {
                    $existing_images = array(
                        'main' => $gallery_row['Image1'],
                        'img1' => $gallery_row['Image2'],
    ...
    if (isset($_POST['delete']) && isset($existing_images['gallery_id'])) {
        ...
        $delete_query = "DELETE FROM DestinationGallery  WHERE ID = $gallery_id";
    ...
    if (isset($_POST['deactivate']) && isset($existing_images['gallery_id'])) {
        $gallery_id = $existing_images['gallery_id'];
        $deactivate_query = "UPDATE DestinationGallery  SET Status = 'inactive' WHERE ID = $gallery_id";
    ...
    <div class="content-wrapper">
        ...
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Upload Destination Images of The Set Destination.</h4>
                        
                        <?php echo $msg; ?>
                        
                        <form class="forms-sample" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?destid=' . $destid; ?>" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="description" placeholder="<?php echo "$'gallery_desc'"?>" >
                            </div>
